...
./appimagetool --comp zstd --mksquashfs-opt -Xcompression-level --mksquashfs-opt 20 \
  -s -u "gh-releases-zsync|$GITHUB_REPOSITORY_OWNER|DeaDBeeF-AppImage|continuous|*Nightly*$ARCH.AppImage.zsync" \
  ./"$APP".AppDir DeaDBeeF_Nightly-"$VERSION"-"$ARCH".AppImage 
